...
const uploadCompanyLogo = async (req, res) => {
    ...
    cloudinary.uploader.upload(req.file.path, async (result, error) => {
        try {
            ...
            const data = await companyModel.findByIdAndUpdate(req.params.id, {...urlToUpdate})
            if (!data) {
                fs.unlinkSync(req.file.path)
            ...
            fs.unlinkSync(req.file.path)
            res.status(500).json({ msg: "Error uploading or updating company logo", error: error.message })
        }
    }
)}
